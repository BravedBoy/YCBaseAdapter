...
import com.yc.cn.ycbaseadapterlib.second.cell.EmptyCell;
import com.yc.cn.ycbaseadapterlib.second.cell.ErrorCell;
import com.yc.cn.ycbaseadapterlib.second.cell.LoadMoreCell;
import com.yc.cn.ycbaseadapterlib.second.cell.LoadingCell;
 ...
 * 版    本：1.0
 * 创建日期：2016/5/9
 * 描    述：RecycleView的adapter
 * 修订历史：
 * ================================================
 */
public class SimpleRvAdapter extends BaseRvAdapter {
    ...
    public void onViewAttachedToWindow(RecyclerView.ViewHolder holder) {
        ...
        if(isStaggeredGridLayout(holder)){
            if(viewType == ERROR_TYPE|| viewType == EMPTY_TYPE || viewType == LOADING_TYPE ||viewType == LOAD_MORE_TYPE){
    ...
    public void showLoadingKeepCount(int keepCount,int height,View loadingView){
        if(keepCount < 0 || keepCount>getData().size()){
            return;
        }
        remove(keepCount,getData().size() - keepCount);
    ...
    public void hideLoading(){
        if(getData().contains(mLoadingCell)){
            getData().remove(mLoadingCell);
    ...
    public void hideErorr(){
        if(getData().contains(mErrorCell)){
            remove(mErrorCell);
            mIsShowError = false;
        }
    }
    ...
    public void showLoadMore(){
        if(getData().contains(mLoadMoreCell)){
            return;
        }
        checkNotContainSpecailCell();
        add(mLoadMoreCell);
    ...
    public void showLoadMore(View loadMoreView,int height){
        ...
        //设置默认高度
        if(height == 0){
            int defaultHeight = dpToPx(loadMoreView.getContext(),LoadMoreCell.mDefaultHeight);
    ...
    public void hideEmpty(){
        if(getData().contains(mEmptyCell)){
            remove(mEmptyCell);
            mIsShowEmpty = false;
        }
    }
    ...
    private void checkNotContainSpecailCell(){
        if(getData().contains(mEmptyCell)){
            getData().remove(mEmptyCell);
        }
        if(getData().contains(mErrorCell)){
            getData().remove(mErrorCell);
        }
        if(getData().contains(mLoadingCell)){
            getData().remove(mLoadingCell);
        }
        if(getData().contains(mLoadMoreCell)){
            getData().remove(mLoadMoreCell);
        }
    }
